...
- 提供 1–49 流体舒尔特，用圆角不规则分区打破传统横竖扫描路径。
...
传入 `--follow true` 可生成节奏跟练版：画面逐个提示当前目标，先留出自主寻找时间，再缓慢亮出正确位置。跟练版目前支持经典 `grid` 布局，不能与亲子版或定时重排同时使用。`--follow-step-seconds 2` 可把每个数字设为 2 秒，并自动把正式时长调整为数字总数 × 2 秒；`--follow-hint-at 0.5` 表示每一步进行到 50% 时开始渐亮，可设置为 `0.2` 到 `0.85`。
