...
语音内容会根据玩法、数字范围、正序/倒序、旋转和刷新间隔自动生成。默认通过 macOS 语音框架使用较自然的 `Li-Mu` 普通话男声和 175 语速；也可通过 `--voice-name Yu-shu` 切换为女声，或使用 `--voice-name Tingting` 保留旧音色。`--voice-rate` 可调整语速，传入 `--voice-text` 可以完全覆盖自动文案。默认在语音进行到 50% 时切换到倒计时画面，可用 `--voice-countdown-at 0.5` 调整切换比例。语音会经过轻量 EQ 和动态压缩；背景音乐从倒计时开始播放，并在人声结束前自动降低音量。
